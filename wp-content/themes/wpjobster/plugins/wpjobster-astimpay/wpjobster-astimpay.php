<?php
/*
	Plugin Name: AstimPay Plugin
	Plugin URL: https://astimpay.com
	Description: AstimPay
	Version: 1.0.0
	Author: OPSYT DEVELOPER
	Author URI: https://opsytdeveloper.eu.org
*/
...
if (!class_exists("WPJobster_AstimPay_Loader")) {

    class WPJobster_AstimPay_Loader
    {
        ...
        public function __construct($gateway = 'astimpay')
        {

            $this->priority   = 200;
            $this->_unique_id = 'astimpay';
        ...
        function get_gateway_currency($currency)
        {
            ...
            $accepted_astimpay_currencies = array('BDT');

            // if currency not supported
            if (!in_array($currency, $accepted_astimpay_currencies)) {
                ...
                foreach ($wpjobster_currencies_array as $wpjobster_currency) {
                    if (in_array($wpjobster_currency, $accepted_astimpay_currencies)) {
        ...
        {
            if (get_option('wpjobster_astimpay_api_key')) {
                $methods[$this->priority] = array(
                    'label'           => __('AstimPay', 'wpjobster'),
        ...
        {
            if (isset($_POST['wpjobster_save_' . $this->_unique_id])) {
                update_option('wpjobster_astimpay_enable', trim($_POST['wpjobster_astimpay_enable']));
                update_option('wpjobster_astimpay_api_key', trim($_POST['wpjobster_astimpay_api_key']));
                update_option('wpjobster_astimpay_api_url', trim($_POST['wpjobster_astimpay_api_url']));
                update_option('wpjobster_astimpay_button_caption', trim($_POST['wpjobster_astimpay_button_caption']));

                if (wpj_get_payment_types()) {
                    foreach (wpj_get_payment_types() as $payment_type_enable_key => $payment_type_enable) {
                        if (isset($_POST['wpjobster_astimpay_enable_' . $payment_type_enable_key]))
                            update_option('wpjobster_astimpay_enable_' . $payment_type_enable_key, trim($_POST['wpjobster_astimpay_enable_' . $payment_type_enable_key]));
                    }
                }
                ...
                <form method="post" action="<?php bloginfo('url'); ?>/wp-admin/admin.php?page=payment-methods&active_tab=tabs<?php echo $tab_id; ?>">
                    <table width="100%" class="wpj-admin-table">
                        <tr>
                            ...
                            <td><?php echo wpjobster_get_option_drop_down($arr, 'wpjobster_astimpay_enable', 'no'); ?></td>
                        </tr>
                        ...
                        <?php foreach (wpj_get_payment_types() as $key => $payment_type) { ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo wpjobster_get_option_drop_down($arr, 'wpjobster_astimpay_enable_' . $key); ?></td>
                            </tr>
                        ...
                        <tr>
                            ...
                            <td><input type="text" size="45" name="wpjobster_astimpay_api_key" value="<?php echo apply_filters('wpj_sensitive_info_email', get_option('wpjobster_astimpay_api_key')); ?>" /></td>
                        </tr>

                        <tr>
                            <td valign="top" width="22"><?php wpjobster_theme_bullet(); ?></td>
                            <td><?php _e('API URL:', 'wpjobster'); ?></td>
                            <td><input type="text" size="45" name="wpjobster_astimpay_api_url" value="<?php echo apply_filters('wpj_sensitive_info_email', get_option('wpjobster_astimpay_api_url')); ?>" /></td>
                        </tr>

                        <tr>
                            <td valign="top" width="22"><?php wpjobster_theme_bullet("Put the AstimPay Button caption you want user to see on purchase page "); ?></td>
                            <td><?php _e('AstimPay Button caption:', 'wpjobster'); ?></td>
                            <td><input type="text" size="45" name="wpjobster_astimpay_button_caption" value="<?php echo get_option('wpjobster_astimpay_button_caption'); ?>" /></td>
                        </tr>

                        <tr>
                            <td valign="top" width="22"><?php wpjobster_theme_bullet("Please select a page to show when astimpay payment successful."); ?></td>
                            <td><?php _e('Transaction success page:', 'wpjobster'); ?></td>
                            <td><?php echo wpjobster_get_option_drop_down($arr_pages, 'wpjobster_astimpay_success_page', '', ' class="" '); ?></td>
                        </tr>

                        <tr>
                            <td valign="top" width="22"><?php wpjobster_theme_bullet(); ?></td>
                            <td><?php _e('Transaction failure page:', 'wpjobster'); ?></td>
                            <td><?php echo wpjobster_get_option_drop_down($arr_pages, 'wpjobster_astimpay_failure_page', '', ' class="" '); ?> </td>
                        </tr>

                        <tr>
                            <td></td>
                            <td></td>
                            <td><input type="submit" name="wpjobster_save_astimpay" value="<?php _e('Save Options', 'wpjobster'); ?>" /></td>
                        </tr>
...
<?php }
        ...
        {
            $this->api_key = get_option('wpjobster_astimpay_api_key');
            if (empty($this->api_key)) {
                echo __("ERROR: please input your astimpay api key address in backend", 'wpjobster');
                exit;
            }
            ...
            $webhook_url        = get_bloginfo('url') . '/?payment_response=astimpay&oid=' . $order_id . '&wpj_payment_id=' . $payment->id . '&action=ipn';
            $success_page       = get_bloginfo('url') . '/?payment_response=astimpay&oid=' . $order_id . '&wpj_payment_id=' . $payment->id . '&action=success';
            $cancel_page        = get_bloginfo('url') . '/?payment_response=astimpay&payment_type=' . $payment_type . '&action=cancel&order_id=' . $order_id . '&jobid=' . $pid . '&wpj_payment_id=' . $payment->id;
            ...
            $result = AstimPay_Gateway_API_Handler::create_payment(
                $amount,
                $currency_code,
                $full_name,
                $email,
        ...
        protected function init_api()
        {
            ...
            include_once dirname(__FILE__) . '/lib/class-astimpay-api-handler.php';

            AstimPay_Gateway_API_Handler::$api_url = trim(get_option('wpjobster_astimpay_api_url'));
            AstimPay_Gateway_API_Handler::$api_key = trim(get_option('wpjobster_astimpay_api_key'));
        }
        ...
        public function processgateway_function($payment_type, $details)
        {
            ...
            if (isset($_GET['action']) && $_GET['action'] === 'success') {
                ...
                $result = AstimPay_Gateway_API_Handler::verify_payment($invoice_id);
        ...
        private function up_process_payment($response_decoded)
        {
            ...
            if (isset($response_decoded->metadata->payment_id) && isset($response_decoded->status) && isset($response_decoded->sender_number) && isset($response_decoded->transaction_id)) {
                ...
                if (strtolower($payment_status) == 'completed') {
                    $payment_details = sprintf(__('AstimPay Payment Method: %s , AstimPay Transaction ID: %s , AstimPay Phone Number: %s', 'wpjobster'), $payment_method, $transaction_id, $phone_number);
                    ...
                    $payment_details = sprintf(__('Payment not cleared yet, may need manual action through AstimPay. Pending reason: %s', 'wpjobster'), 'Amount is not matching');
                    $this->debug_log('invalid');
                    $this->other($payment_type, $order_id, $payment_details, $payment_response, 'processing');
                } else {
                    $payment_details = sprintf(__('Payment not cleared yet, may need manual action through AstimPay. Pending reason: %s', 'wpjobster'), 'Transaction ID Not Found');
        ...
        /**
         * Check AstimPay Gateway webhook request is valid.
         * @param  string $data
         */
        public function validate_webhook()
        {
            ...
            $key = 'HTTP_' . strtoupper(str_replace('-', '_', 'API-KEY'));
            ...
            $api_key = trim(get_option('wpjobster_astimpay_api_key'));
        ...
        public function debug_log($text)
        {
            ...
            $fp = fopen('.wpj_astimpay.log', 'a');
            fwrite($fp, $text . "\n");
            fclose($fp);
        }
    } // END CLASS

    $GLOBALS['WPJobster_AstimPay_Loader'] = WPJobster_AstimPay_Loader::get_instance();
} // END IF CLASS EXIST